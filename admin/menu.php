			...
			<form action="" id="manage-menu">
				<div class="card">
					<div class="card-header">
						    <span id="menu-form-title">New Product</span>
				  	</div>
					<div class="card-body">
							...
							<div class="form-group">
								<img src="" alt="" id="menu-preview-img" class="cimg">
							</div>
					</div>
					...
					<div class="card-footer">
						<div class="row">
							<div class="col-md-12">
								...
								<button class="btn btn-sm btn-default col-sm-3" type="button" id="cancel_menu"> Cancel</button>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="col-md-8">
				<div class="card">
					<div class="card-header d-flex justify-content-between align-items-center">
						<b>Menu List</b>
						<button class="btn btn-sm btn-success" type="button" id="add_new_menu"><i class="fa fa-plus"></i> Add New</button>
					</div>
					<div class="card-body">
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-center">
										<img src="<?php echo isset($row['img_path']) ? '../assets/img/'.$row['img_path'] :'' ?>" alt="" class="cimg menu-thumb">
									</td>
									<td class="">
									...
									<td class="text-center">
										<button class="btn btn-sm btn-primary edit_menu" type="button" data-id="<?php echo $row['id'] ?>">Edit</button>
...
<script>
	...
	function resetMenuForm(){
		...
		$('#menu-preview-img').attr('src', '')
		$('#menu-form-title').text('New Product')
	}
	$('#add_new_menu, #cancel_menu').click(function(){
		resetMenuForm()
		$('html, body').animate({ scrollTop: $('#manage-menu').offset().top - 80 }, 300)
	})
	...
	$(document).on('click', '.edit_menu', function(){
		...
		start_load()
		$.ajax({
			url: 'ajax.php?action=get_menu',
			method: 'POST',
			data: { id: btn.data('id') },
			dataType: 'json',
			success: function(data){
				if(!data || !data.id){
					alert_toast('Unable to load menu item','danger')
					end_load()
					return
				}
				var form = $('#manage-menu')
				form.get(0).reset()
				$('#menu-form-title').text('Edit Product')
				form.find("[name='id']").val(data.id)
				form.find("[name='name']").val(data.name)
				form.find("[name='description']").val(data.description)
				form.find("[name='price']").val(data.price)
				form.find("[name='category_id']").val(data.category_id)
				$('#availability').prop('checked', parseInt(data.status) === 1)
				if(data.img_path){
					$('#menu-preview-img').attr('src', '../assets/img/' + data.img_path)
				} else {
					$('#menu-preview-img').attr('src', '')
				}
